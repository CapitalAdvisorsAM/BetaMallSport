...
export type ExistingUnitForDiff = {
  codigo: string;
  nombre: string;
  glam2: string | number;
  piso: string;
...
function emptyRow(): UnitUploadRow {
  return {
    codigo: "",
    nombre: "",
    glam2: "",
...
function summarize(rows: PreviewRow<UnitUploadRow>[]): UploadPreview<UnitUploadRow>["summary"] {
  return rows.reduce(
    (summary, row) => {
      summary.total += 1;
      if (row.status === "NEW") {
...
function compareWithExisting(
  row: UnitUploadRow,
  existing: ExistingUnitForDiff
): (keyof UnitUploadRow)[] {
  const changed: (keyof UnitUploadRow)[] = [];
...
export function parseUnitsFile(
  buffer: ArrayBuffer,
  existingMap: Map<string, ExistingUnitForDiff>
): UploadPreview<UnitUploadRow> {
  const workbook = read(Buffer.from(buffer), { type: "buffer", raw: false, cellDates: false });
  const firstSheet = workbook.SheetNames[0];
  if (!firstSheet) {
    const rows: PreviewRow<UnitUploadRow>[] = [
      {
        rowNumber: 0,
        status: "ERROR",
        data: emptyRow(),
  ...
  const previewRows: PreviewRow<UnitUploadRow>[] = normalizedRows.map((rawRow, index) => {
