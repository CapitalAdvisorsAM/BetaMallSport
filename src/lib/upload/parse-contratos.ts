...
export type ContratoPreviewInputRow = {
  rowNumber: number;
...
export function buildContratoLookupKey(input: {
  ...
  return [
    ...
    normalizeUploadArrendatarioNombre(input.arrendatarioNombre ?? ""),
    input.fechaInicio ?? "",
    input.fechaTermino ?? ""
  ].join("|");
}
...
export function normalizeUploadArrendatarioNombre(value: string): string {
  return value
    .trim()
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
...
function compareWithExisting(
  ...
  if (
    normalizeUploadArrendatarioNombre(existing.arrendatarioNombre) !==
...
export function revalidateContratoPreviewRows(
  rows: ContratoPreviewInputRow[],
  options: ParseContratosOptions
