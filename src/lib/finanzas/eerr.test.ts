...
import { buildEerrData, calculateEbitdaMargin, COST_GROUPS } from "@/lib/finanzas/eerr";
...
describe("buildEerrData", () => {
  it("groups sections and lines by period while calculating EBITDA", () => {
    ...
    expect(result.ebitda.porPeriodo["2026-01"]).toBe(110);
    expect(result.ebitda.total).toBe(190);
  });
});
