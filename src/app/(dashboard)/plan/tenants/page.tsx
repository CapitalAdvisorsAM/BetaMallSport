...
import { TenantsCrudPanel } from "@/components/plan/TenantsCrudPanel";
import { TenantsViewTable } from "@/components/plan/TenantsViewTable";
...
import { TenantsPeriodPicker } from "@/components/plan/TenantsPeriodPicker";
...
export default async function TenantsPage({
    ...
    [tenantsList, totalTenants, totalPeriodo, totalVigentesPeriodo, totalContratosPeriodo] =
      await Promise.all([
        ...
        prisma.contract.count({
          where: { projectId: selectedProjectId, ...activeContractWhere }
        })
      ]);
  } else if (mode === "cargar") {
  ...
  const buildPageHref = (page: number): string => {
    ...
    return `/plan/tenants?${params.toString()}`;
  };
