...
import { OccupancyBadge } from "@/components/real/OccupancyBadge";
...
import { getGapSeverity } from "@/lib/real/gap-utils";
...
export function FinanceTenantsClient({
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/real/tenants?${params.toString()}`);
  ...
  async function togglePartida(tenantId: string, periodo: string): Promise<void> {
    ...
    try {
      ...
      const res = await fetch(`/api/real/tenants/detail?${params.toString()}`);
                    ...
                    <tr className={`${getStripedRowClass(index)} ${tableTheme.rowHover}`}>
                      ...
                      <td className="px-3 py-3 text-slate-500">
                        {tenant.locales.map((local, i) => (
                          <span
                            key={local.id}
                            onClick={() => router.push(`/plan/units?detalle=${local.id}`)}
                            className={cn(
