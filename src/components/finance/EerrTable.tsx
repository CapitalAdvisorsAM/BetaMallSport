...
import { calculateEbitdaMargin } from "@/lib/finance/eerr";
import {
  Table,
  TableBody,
  TableCell,
...
function getEerrValueTone(tipo: "ingreso" | "costo", value: number): string {
          ...
          <TableBody>
            {data.secciones.map((section) => (
              <Fragment key={section.grupo1}>
                ...
                {/* Detail lines â€” visible when section is expanded */}
            ...
            {/* EBITDA margin row â€” only shown when there are income sections */}
              ...
              <TableRow className="bg-brand-700/5 hover:bg-brand-700/5">
                ...
                {data.periodos.map((periodo) => {
                  ...
                  return (
                    ...
                    >
                      {margin !== null ? `${formatUf(margin, 1)}%` : "â€”"}
                    </TableCell>
                  );
                })}
