...
function getFileName(item: UploadHistoryItem): string {
  return item.fileName ?? item.archivoNombre ?? "-";
}

function getStatus(item: UploadHistoryItem): string {
...
}: UploadHistoryProps): JSX.Element {
  const columns = useMemo<ColumnDef<UploadHistoryItem, unknown>[]>(
    () => [
      {
        id: "createdAt",
